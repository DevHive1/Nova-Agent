// ============================================================
//  server/lib/performance.js    Performance Monitoring and Optimization
// ============================================================

const { performance, PerformanceObserver } = require('perf_hooks');
const os = require('os');
const { isRedisAvailable } = require('./cache');
 ...
 */
function recordCacheAccess(isHit) {
  if (isHit) {
    metrics.cacheHits++;
  } else {
    metrics.cacheMisses++;
 ...
 */
function recordToolCall(toolName, duration) {
  metrics.toolCalls[toolName] = (metrics.toolCalls[toolName] || 0) + 1;
}
 ...
 * @param {number} duration - Duration in ms
 */
function recordOllamaCall(duration) {
  metrics.ollamaCalls++;
  metrics.ollamaAvgTime = (
